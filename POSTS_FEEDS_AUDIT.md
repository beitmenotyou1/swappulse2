...
| 🟠 P1 | PF-023 - Feed subscription uses the wrong DID property | `FeedCard` reads the DID from `user.data?.did` and falls back to an empty string, while other current code treats `user.did` as authoritative. This can create a subscription with an empty DID. | Create subscriptions through a backend function that derives DID from the authenticated user. Reject empty/mismatched DID and validate `feed_uri`. |
...
| 🟢 P3 | PF-038 - Subscriber counts are not clearly authoritative | Seed feed cards fall back to a displayed subscriber count of zero when `subscriberCount` is absent, but the static seed objects do not carry an authoritative count and current `FeedSubscription` data is empty. Users may see zero rather than an explicit unavailable state. | Return counts from the authoritative feed/subscription service or label them unavailable instead of implying a measured zero. |

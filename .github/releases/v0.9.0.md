...
SwapPulse does not guess Impact deep-link syntax. Until an approved account provides the actual tracking format, all outbound TCGplayer links remain ordinary links.

After approval, `TCGPLAYER_AFFILIATE_URL_TEMPLATE` may be configured with an HTTPS template containing `{url}`. The Base44 backend validates the ordinary TCGplayer destination and then decorates it using the approved template. When tracking is active, SwapPulse displays: `Affiliate link: SwapPulse may earn a commission from qualifying TCGplayer purchases at no extra cost to you.`

Affiliate configuration does not enable the dormant direct TCGplayer API and does not bypass `TCGPLAYER_APPROVED_USE`.

## Security/privacy
- TCGplayer developer keys and Bearer tokens remain backend-only.

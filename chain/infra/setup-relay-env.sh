...
const required = ['account_class_hash', 'identity_registry_address', 'identity_registry_owner'];
...
IDENTITY_REGISTRY_ADDRESS="${VALUES[1]:-}"
IDENTITY_REGISTRY_OWNER="${VALUES[2]:-}"
RECOVERY_CONTROLLER="${VALUES[3]:-0x0}"
RECOVERY_DELAY_SECONDS="${VALUES[4]:-172800}"
...
umask 077
cat > "$OUT" <<EOF
SWAPPULSE_RELAY_TOKEN=$RELAY_TOKEN
SWAPPULSE_RELAY_ACCOUNT_CLASS_HASH=$ACCOUNT_CLASS_HASH
SWAPPULSE_RELAY_IDENTITY_REGISTRY_ADDRESS=$IDENTITY_REGISTRY_ADDRESS
SWAPPULSE_RELAY_IDENTITY_REGISTRY_OWNER=$IDENTITY_REGISTRY_OWNER
SWAPPULSE_RELAY_RECOVERY_CONTROLLER=$RECOVERY_CONTROLLER
SWAPPULSE_RELAY_RECOVERY_DELAY_SECONDS=$RECOVERY_DELAY_SECONDS
SWAPPULSE_RELAY_MINT_AMOUNT=5000000000000000
SWAPPULSE_RELAY_RATE_LIMIT=60
EOF
chmod 600 "$OUT"

echo "Relay environment written to $OUT with mode 0600."
echo "Copy SWAPPULSE_RELAY_TOKEN only into Base44 server-side secrets, never into browser code or ChainNetworkConfig."

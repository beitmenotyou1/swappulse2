  ...
  try {
    ...
    switch (feed) {
      ...
      case 'market-watch': {
        ...
        return Response.json({
          feed: movers.map((m) => ({ post: m.p.at_uri || `at://swappulse/pricing/${m.p.id}`, reason: `mover_${m.pct > 0 ? 'up' : 'down'}`, feedContext: String(m.pct.toFixed(1)) })),
      ...
      case 'smart-bundles': {
        ...
        const mine = await svc.entities.CollectionEntry.list('-updated_date', 500);
        const dupes = new Map();
        for (const c of mine) {

...
// Bulk-mints multiple cards from the collector's CollectionEntry as
// transferable ERC-721 NFTs on Polygon. Each card requires a proof photo
// URL (captured by the collector before minting). Uses TCGDex card images
// as the NFT visual, and embeds the minter's username in the metadata.
//
// Gas for all mints is paid by the collector (deducted from their wallet
// or via the platform mint wallet, depending on configuration).
//
...
export default async function(req: Request): Promise<Response> {
  try {
    ...
    const { items, unlockCredential } = body;
    if (!Array.isArray(items) || items.length === 0) {
      return Response.json({ error: 'No cards selected for minting' }, { status: 400 });
    }
    if (items.length > 20) {
      return Response.json({ error: 'Maximum 20 cards per bulk mint' }, { status: 400 });
    }

    const did = user.did || user.data?.did;
    ...
    if (settingsList.length) {
      ...
      if (cryptoConfig && cryptoConfig.enabled === false) {
        return Response.json({ error: 'Crypto features are disabled. Enable them in Settings.' }, { status: 403 });
      }
    }
    ...
    // Validate all items and check for existing mints
    const validatedItems = [];
    const alreadyMinted = [];
    for (const item of items) {
      const { collectionEntryId, proofPhotoUrl, verificationSessionId } = item;
      if (!collectionEntryId || !proofPhotoUrl) {
        return Response.json({ error: 'Each card requires a collectionEntryId and proofPhotoUrl' }, { status: 400 });
      }

      // Fetch the collection entry
      const entries = await base44.entities.CollectionEntry.filter({ id: collectionEntryId });
      if (!entries.length) {
        return Response.json({ error: `Collection entry ${collectionEntryId} not found` }, { status: 404 });
      }
      ...
      if (existing.length) {
        alreadyMinted.push({ collectionEntryId, asset: existing[0] });
        continue;
      }

      // Validate verification session if provided
      ...
      if (verificationSessionId) {
        ...
        if (sessions.length) {
          ...
          if (session.collection_entry_id !== collectionEntryId) continue;
    ...
    if (validatedItems.length === 0) {
      return Response.json({
        error: 'All selected cards are already minted',
        alreadyMinted,
      }, { status: 400 });
    }
    ...
    // Mint all validated cards via the dual-mint engine
    const results = [];

    for (const item of validatedItems) {
      try {
        const { entry, verificationSessionId, verificationLevel } = item;
        ...
        const dualResult = await mintCardDual(
          ...
          verificationLevel,
          did,
          { primaryChain, bridgeToSecondary, collectionEntryId: entry.id, verificationSessionId },
        );
        ...
        results.push({
          ...
          cardName,
          asset: dualResult.polygonAsset || dualResult.pulseAsset,
          txHash: dualResult.polygonTxHash || dualResult.pulseTxHash,
          polygonTxHash: dualResult.polygonTxHash,
          pulseTxHash: dualResult.pulseTxHash,
          bridgeStatus: dualResult.bridgeStatus,
        });
      } catch (mintError: any) {
        results.push({
          success: false,
          collectionEntryId: item.entry.id,
          cardName: item.entry.card_name,
          error: mintError.message || 'Mint failed',
        });
      }
    }
    ...
    return Response.json({
      results,
      alreadyMinted,
      totalMinted: results.filter(r => r.success).length,

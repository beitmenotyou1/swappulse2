...
export default function TradeAssistant() {
  ...
  const createNew = async () => {
    try {
      const conv = await base44.agents.createConversation({
        ...
        metadata: { name: 'Trade chat', description: 'Refine offers & negotiate terms' },
      });
  ...
  const send = async () => {
    ...
    try {
      ...
      // The subscription will stream the assistant response
    } catch (e) {
  ...
  return (
    <div>
      <PageHeader title="Trade Assistant" subtitle="Refine your offers and negotiate with AI guidance">
        <Link to="/trades" className="flex items-center gap-1.5 text-sm font-semibold text-muted-foreground hover:text-foreground">
          <ArrowLeft className="h-4 w-4" /> Trade Board
        </Link>
      </PageHeader>
            ...
            <div className="space-y-1">
              {conversations.map((c) => (
                ...
                >
                  ...
                  <span className="truncate">{c.metadata?.name || 'Trade chat'}</span>
                </button>
              ))}
            </div>
          )}
        ...
        <div className="flex min-w-0 flex-1 flex-col rounded-2xl border border-border bg-card">
          {!activeId ? (
            <div className="flex flex-1 flex-col items-center justify-center gap-4 p-8 text-center">
              ...
              <div>
                <h2 className="text-lg font-bold">Trade Assistant</h2>
                <p className="mt-1 max-w-sm text-sm text-muted-foreground">
                  Get help refining your trade listings, checking fairness, and drafting negotiation messages.
                </p>
              </div>
            ...
            <>
              ...
              <div className="sticky bottom-0 border-t border-border bg-card p-3">
                ...
                <p className="mt-1.5 px-1 text-[11px] text-muted-foreground">
                  The assistant can read information you are authorised to see, check market prices, and draft trade changes for you to apply yourself.
                </p>
              </div>
            </>
          )}
      ...
      <DocumentationLink slug="trade-assistant" />
    </div>
  );
}
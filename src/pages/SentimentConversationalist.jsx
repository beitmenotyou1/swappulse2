...
export default function SentimentConversationalist() {
  ...
  const createNew = async () => {
    try {
      const conv = await base44.agents.createConversation({
        ...
        metadata: { name: 'Sentiment pulse check', description: 'Spot trending cards & spark community chats' },
      });
  ...
  return (
    <div>
      <PageHeader title="Sentiment Conversationalist" subtitle="AI-powered community engagement from market sentiment trends">
        <Link to="/" className="flex items-center gap-1.5 text-sm font-semibold text-muted-foreground hover:text-foreground">
            ...
            <div className="space-y-1">
              {conversations.map((c) => (
                ...
                >
                  ...
                  <span className="truncate">{c.metadata?.name || 'Sentiment pulse check'}</span>
                </button>
              ))}
            </div>
          )}
        ...
        <div className="flex min-w-0 flex-1 flex-col rounded-2xl border border-border bg-card">
          {!activeId ? (
            <div className="flex flex-1 flex-col items-center justify-center gap-4 p-8 text-center">
              ...
              <div>
                <h2 className="text-lg font-bold">Sentiment Conversationalist</h2>
                <p className="mt-1 max-w-sm text-sm text-muted-foreground">
                  Spot trending cards from community sentiment votes, draft engaging conversation starters, and post them to the feed to spark discussion.
                </p>
              </div>
            ...
            <>
              ...
              <div className="sticky bottom-0 border-t border-border bg-card p-3">
                ...
                <p className="mt-1.5 px-1 text-[11px] text-muted-foreground">
                  The agent reads sentiment polls & votes, identifies trending cards, and can draft and post conversation starters to the feed.
                </p>
              </div>
            </>
          )}

...
export default function MarketWatchAssistant() {
  useSEO({
    title: 'Market Watch Assistant',
    description: 'AI assistant for tracking Pokémon TCG card prices and market trends on SwapPulse.',
    canonicalPath: '/market-watch-assistant',
  });
  ...
  const createNew = async () => {
    try {
      const conv = await base44.agents.createConversation({
        ...
        metadata: { name: 'Market Watch', description: 'Find wishlist cards on the market' },
      });
  ...
  return (
    <div>
      <PageHeader title="Market Watch" subtitle="AI agent that monitors listings for your Wishlist cards">
        <Link to="/market" className="flex items-center gap-1.5 text-sm font-semibold text-muted-foreground hover:text-foreground">
            ...
            <div className="space-y-1">
              {conversations.map((c) => (
                ...
                >
                  ...
                  <span className="truncate">{c.metadata?.name || 'Market Watch'}</span>
                </button>
              ))}
            </div>
          )}
        ...
        <div className="flex min-w-0 flex-1 flex-col rounded-2xl border border-border bg-card">
          {!activeId ? (
            <div className="flex flex-1 flex-col items-center justify-center gap-4 p-8 text-center">
              ...
              <div>
                <h2 className="text-lg font-bold">Market Watch Agent</h2>
                <p className="mt-1 max-w-sm text-sm text-muted-foreground">
                  Ask about cards on your Wishlist, find active listings that match, and get alerted when new ones appear.
                </p>
              </div>
            ...
            <>
              ...
              <div className="sticky bottom-0 border-t border-border bg-card p-3">
                ...
                <p className="mt-1.5 px-1 text-[11px] text-muted-foreground">
                  The agent can read your Wishlist, scan market listings, and trigger alerts.
                </p>
              </div>
            </>
          )}
      ...
      <DocumentationLink slug="market-watch-assistant" />
    </div>
  );
}
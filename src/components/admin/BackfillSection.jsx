...
// Admin tool to backfill existing local Follow records to the AT Protocol PDS.
// Idempotent — safe to run repeatedly until failed=0.
export default function BackfillSection() {
  ...
  const run = async () => {
    ...
    setResult(null);
    try {
      const res = await base44.functions.invoke('backfill-follows', {});
      const data = res?.data || res;
      setResult(data);
      toast({
        title: 'Backfill complete',
        description: `${data.bridged || 0} bridged, ${data.skipped || 0} skipped, ${data.failed || 0} failed`,
      });
    } catch (err) {
      toast({
      ...
      {result && (
        <div className="mt-3 grid grid-cols-3 gap-2 text-sm">
          <div className="rounded-lg bg-secondary p-2 text-center">
            <p className="font-bold text-primary">{result.bridged || 0}</p>
            <p className="text-xs text-muted-foreground">Bridged</p>
          </div>
          <div className="rounded-lg bg-secondary p-2 text-center">

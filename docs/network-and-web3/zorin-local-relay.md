---
description: >-
  Historical local relay setup and its relationship to the current mini PC
  runtime.
---
...
For moving the already-deployed testnet from the Zorin development PC to the always-on mini PC, follow [mini PC migration guide](https://swappulse.gitbook.io/swappulse-docs/network-and-web3/mini-pc-migration). That migration preserves the existing Devnet state and contract addresses instead of redeploying a replacement chain.
...
Use the official Docker, Cloudflare, Scarb and Starknet Foundry installation instructions when a tool is missing. Keep the Cairo/Foundry versions pinned to the values in [chain overview](https://swappulse.gitbook.io/swappulse-docs/network-and-web3/chain-overview) while this testnet milestone is active.

...
THIRD_PARTY_NOTICES.md
  ...
  dependency/licence notices where required
  Pokémon/Nintendo/Creatures/GAME FREAK ownership disclaimer
  Base44/AT Protocol/Starknet/OpenZeppelin attribution boundaries

README.md
  concise licensing and non-affiliation notice

docs/LICENSING_OPTIONS.md
  rationale/history of licence choice
```

Potentially use a separate documentation licence only if there is a strong reason. Keeping code and original project documentation under the same Apache-2.0 terms is simpler unless media/document reuse needs differ.

...
Approved project knowledge now carries source provenance and is separate from quarantined revisions. A daily fixed-allowlist check can queue changed GitHub and GitBook sources, but an administrator must approve a revision before the Copilot can read it. Feedback-derived agent insights are also inactive until reviewed. The planned 1-to-10 unknown-card scanner and batch-add confirmation flow are not included in this foundation.
